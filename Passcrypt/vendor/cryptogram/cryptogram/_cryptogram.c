 ...
 *
 * Uses OpenSSL (loaded at runtime via dlopen, no build-time headers needed)
 * for hardware-accelerated AES-NI block ops.
...
static void ssl_load(void) {
    if (G.ok) return;
#ifdef __APPLE__
    /* macOS hardened-runtime policy: dlopen a non-system libcrypto triggers
     * a deliberate SIGABRT. Setting ok=-1 makes PyInit__cryptogram raise
     * ImportError so __init__.py falls back to the pure-Python backend. */
    G.ok = -1;
    return;
#endif
     ...
     * auditwheel bundles libcrypto into <pkg>.libs/ and patches the RPATH,
     * so the dynamic linker loads it at import time into the global namespace.
     * dlopen(NULL) reaches those symbols without needing the mangled filename. */
    {
     ...
     * Covers first-import via unusual Python embeddings where libcrypto
     * was bundled by auditwheel but not yet in the process namespace. */
    {
        Dl_info info;
    ...
    /* Strategy 3: bare name search (system install / dev environment) */
    {
        static const char *libs[] = {
             ...
             * (manylinux2014_i686); without it the i686 build falls back to
             * pure-Python and fails if cryptography is absent. */
...
/* CTR-256: symmetric. Batches counter encryption via EVP ECB for throughput. */
#define CTR_BATCH 64
...
/* PQ Factorisation - deterministic Brent-Pollard rho */
...
#else
/* Portable fallback for MSVC x86 and platforms without 128-bit support. */
static uint64_t mulmod(uint64_t a,uint64_t b,uint64_t m){
    uint64_t r=0;
    a%=m;
    while(b){
...
/* PyPI Windows wheels are MSVC; GCC-only __get_cpuid left has_aesni stuck at 0. */
# include <intrin.h>
static int _has_aesni(void){
    int regs[4];
    __cpuid(regs, 1);
...
PyMODINIT_FUNC PyInit__cryptogram(void) {
    ssl_load();
    if (G.ok != 1) {
        /* Raise ImportError so __init__.py can fall back to _fallback.py
         * instead of importing successfully and crashing on first use. */
        PyErr_SetString(PyExc_ImportError,
            "cryptogram: OpenSSL shared library not found - "
