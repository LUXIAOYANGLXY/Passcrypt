...
class AEKEProtocol:
    def __init__(self, region_name,access_key_id,secret_key_id,p=None, g=None, key_len=24, sec_level=2048,verbose=True): # 初始化方法
        ...
        self.G = ec.derive_private_key(1, self.ec_curve, default_backend()).public_key()
        self.ec_private_key = ec.generate_private_key(self.ec_curve, default_backend())
        self.ec_public_key = self.ec_private_key.public_key()
        self.pk = self.ec_public_key
        self.sk = self.ec_private_key.private_numbers().private_value
    ...
    def H_prime(self, *args):
        ...
        for arg in args:
            if isinstance(arg, str):
                data_list.append(arg.encode())
            elif isinstance(arg, (bytes, bytearray)):
                data_list.append(arg)
            elif isinstance(arg, int):
                # 将整数转为字节，长度根据数值大小自动计算
                data_list.append(arg.to_bytes((arg.bit_length() + 7) // 8, 'big'))
            else:
                # 对于其他对象（如点对象），尝试调用序列化方法或转为字符串
                data_list.append(str(arg).encode())

        combined_data = b''.join(data_list)
        # 返回原始字节流 (32 字节)
        return SHA256.new(combined_data).digest()

    # H
    def H_double_prime(self, *args):
        data_list = []
        for arg in args:
            if isinstance(arg, str):
                data_list.append(arg.encode())
        ...
        combined_data = b''.join(data_list)
        # 使用 SHA256 并截取你需要的长度
        return SHA256.new(combined_data).digest()[:self.KEY_LEN]
        
    def derive_aes_key(self, shared_secret: int, key_len=32):
        """
        shared_secret: 群元素（int）
        return: AES key bytes
        """
        ss_bytes = shared_secret.to_bytes(
            (shared_secret.bit_length() + 7) // 8,
            'big'
        )
        return hashlib.sha256(ss_bytes).digest()[:key_len]

        def serialize_pubkey(self, pubkey):
        return pubkey.public_bytes(
            encoding=serialization.Encoding.X962,
            format=serialization.PublicFormat.UncompressedPoint
        )

    def deserialize_pubkey(self, data):
        # 调试打印
        print(f"[DEBUG] Raw data for deserialization: {data.hex() if data else 'None'}")
        print(f"[DEBUG] Data length: {len(data) if data else 0}")

        if not data or data[0] not in [2, 3, 4]:
            print("[ERROR] 字节流开头不是有效的 ECC 控制位 (02, 03, 04)")

        return ec.EllipticCurvePublicKey.from_encoded_point(self.ec_curve, data)

    def ecdh_shared_secret(self, peer_pubkey_bytes: bytes):
        peer_pubkey = self.deserialize_pubkey(peer_pubkey_bytes)

        shared_key = self.ec_private_key.exchange(ec.ECDH(), peer_pubkey)
        # shared_key 是 bytes

        # KDF 派生 AES key
        return hashlib.sha256(shared_key).digest()[:self.KEY_LEN]
    ...
    def H_pw_gs(self,gs,pw):
        st=gs
        return st

    def derive_key(self, shared_secret, key_len=None) -> bytes:
        """
        派生 AES key
        shared_secret: int 或 bytes
        """
        if key_len is None:
        ...
        if isinstance(shared_secret, int):
            ...
            ss_bytes = shared_secret
        else:
            raise TypeError("shared_secret must be int or bytes")
        # Hash
        return hashlib.sha256(ss_bytes).digest()[:key_len]
    ...
    def _adjust_key(self, key) -> bytes:
        ...
        if isinstance(key, int):
            try:
                # 尝试直接转换
                key = key.to_bytes(self.KEY_LEN, byteorder='big')
            except OverflowError:
                # 如果太长，通过哈希压缩到固定长度
                key = hashlib.sha256(str(key).encode()).digest()[:self.KEY_LEN]
    ...
    def AES_encrypt_h(self, key, plaintext, header: bytes):
        """
        AES-GCM encryption with AAD (header)
        ...
        print(f"【DEBUG】AES_encrypt nonce: {nonce.hex()}")
        print(f"【DEBUG】AES_encrypt tag: {tag.hex()}")
        print(f"【DEBUG】AES_encrypt header: {header.hex()}")

        # 返回 nonce || tag || ciphertext
        return nonce + tag + ciphertext

    def AES_decrypt_h(self, key, ciphertext_bundle, header: bytes):
        """
        AES-GCM decryption with AAD (header)
        """
        key = self._adjust_key(key)
        print(f"【DEBUG】AES_decrypt key: {key.hex()}")
        nonce = ciphertext_bundle[:12]
        ...
        print(f"【DEBUG】AES_decrypt nonce: {nonce.hex()}")
        print(f"【DEBUG】AES_decrypt tag: {tag.hex()}")
        print(f"【DEBUG】AES_decrypt header: {header.hex()}")
        ...
        # 如果 header / ciphertext / tag 任一被篡改
        # finalize() 会抛 InvalidTag 异常
        return decryptor.update(ciphertext) + decryptor.finalize()

    #-------------------------------------------------------------------------------
    def AES_CTR_encrypt(self, key, plaintext):
        key = self._adjust_key(key)
        print(f"【DEBUG】AES_CTR_encrypt key: {key.hex()}")
        # CTR 模式需要 16 字节 IV（AES block size）
        iv = os.urandom(16)
        encryptor = Cipher(
            algorithms.AES(key),
            modes.CTR(iv),
        ...
        ciphertext = encryptor.update(plaintext) + encryptor.finalize()
        print(f"【DEBUG】AES_CTR_encrypt iv: {iv.hex()}")
        # CTR 无 tag，只返回 iv + ciphertext
        return iv + ciphertext

    def AES_CTR_decrypt(self, key, ciphertext_bundle):
        key = self._adjust_key(key)
        print(f"【DEBUG】AES_CTR_decrypt key: {key.hex()}")
        iv = ciphertext_bundle[:16]
        ciphertext = ciphertext_bundle[16:]
        print(f"【DEBUG】AES_CTR_decrypt iv: {iv.hex()}")
        decryptor = Cipher(
            algorithms.AES(key),
            modes.CTR(iv),
            backend=default_backend()
    ...
    def AES_decrypt_streaming_from_stream(self, key, input_stream, dest_path, k1, chunk_size=1024 * 1024):
        key = self._adjust_key(key)
        print(f"[DEBUG] AES_decrypt_streaming_from_stream key: {key.hex()}")
    ...
    def AES_encrypt_streaming_h(self, key, plaintext_path, inter_enc_path, k2, header, chunk_size=1024 * 1024):
        key = self._adjust_key(key)
        nonce = os.urandom(12)
        encryptor = Cipher(
            algorithms.AES(key),
    ...
    def AES_encrypt_streaming_to_stream_h(self, key, plaintext_path, output_stream, k2, header, chunk_size=1024 * 1024):
        key = self._adjust_key(key)
        nonce = os.urandom(12)
        print(f"[DEBUG] AES_encrypt_streaming_to_stream key: {key.hex()}")
        print(f"[DEBUG] AES_encrypt_streaming_to_stream nonce: {nonce.hex()}")
        ...
        if header is not None:
            ...
            encryptor.authenticate_additional_data(header)
            print(f"[DEBUG] 已绑定 AAD (Header), 长度: {len(header)} 字节")

        # 准备输出流结构: [Nonce (12B)] [Tag (16B)] [Ciphertext...]
        output_stream.write(nonce)  # 写入 nonce
        tag_pos = output_stream.tell()  # 记录 tag 的位置以便后续回写
        output_stream.write(b'\x00' * 16)  # tag 占位

        #  分块加密文件
        with open(plaintext_path, "rb") as fin:
            print("[DEBUG] 开始加密文件流并绑定 AAD...")
            while chunk := fin.read(chunk_size):
                enc_chunk = encryptor.update(chunk)
                output_stream.write(enc_chunk)
        ...
        tag = encryptor.tag
        print(f"[DEBUG] GCM tag: {tag.hex()}")
        current_pos = output_stream.tell()  # 记录末尾位置
        output_stream.seek(tag_pos)
        output_stream.write(tag)
    ...
    def AES_decrypt_streaming_from_stream_h(self, key, input_stream, dest_path, k1, header, chunk_size=1024 * 1024):
        key = self._adjust_key(key)
