    ...
    if role == Constants.CLIENT:
        ...
        for i in range(10):
            ...
            if secure_level == 1:
                ...
                c_path = client_run_enc3(protocol,s, source_file_path,run_time,user_id, passphrase,inter_enc_path,k4,bucket_name,k_client, st,total_bytes)##PAKE+加密+上传   2222
                k_client, st, s,total_bytes = client_run_dec3_PAKE(protocol, run_time, user_id, passphrase,secure_level)
                client_run_dec3(protocol,s, dec_dest_path,k1,run_time,user_id, passphrase,bucket_name,k_client, st,total_bytes,c_path)#PAKE+解密+下载
            # elif secure_level == 4:
